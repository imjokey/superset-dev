...
const AiModalContent = styled.div`
  .chatContent {
    ...
    .progress {
      ...
      background:
       linear-gradient(#20a7c9 0 0) 0/0% no-repeat
       #ddd;
...
const CssTemplateModal: FunctionComponent<CssTemplateModalProps> = ({
  ...
  const getList = () => {
    ...
    fetch("https://hit-mitlab.cn:7860/generate_index", {
      ...
      // aiPost(data)
      let arr = data.map((i: any) => { return { title: i, checkStatus: false } })
      setCheckList([...arr])
      let newArr = list
  ...
  const checkAll = (is: any) => {
    let arr = checkList
    arr = arr.map((i: any) => { return { ...i, checkStatus: is } })
    setCheckList([...arr])
  ...
  return (
    <Modal
      ...
      width="55%"
      title={
        <h4 data-test="css-template-modal-title">AI分析</h4>
      }
    >
      ...
      <AiModalContent>
        <div className='chatContent' ref={divRef}>
          <div className="tipBox">
            <div className="el-upload__tip">当前数据模型，包括以下维度和度量：</div>
            <div className="el-upload__tip">维度：</div>
            <div className="el-upload__tip">1. 销售表， 包括: 销售姓名, 销售分部等信息</div>
            <div className="el-upload__tip">2. 合同表， 包括: 合同名称, 行业名称, 商机类型, 合同类型等信息</div>
            <div className="el-upload__tip">3. 合同明细表， 包括: 商机来源, 是否在保, 是否新客等信息</div>
            <div className="el-upload__tip">4. 合同签订日期_时间维度， 包括: 年, 年季, 年月, 年周, 年月日等信息</div>
            <div className="el-upload__tip">5. 地理维， 包括: 省份, 城市等信息</div>
            <div className="el-upload__tip">度量：</div>
            <div className="el-upload__tip">1. 合同明细表， 包括: 商机预算, 售前成本, 回款金额, 外采费用, 产品模块标价等信息</div>
            <div className="el-upload__tip">2. 其他， 包括: 利润, 项目成本等信息</div>
            <div className="el-upload__tip">示例问句：</div>
            <div className="el-upload__tip tipBoxQusetion" onClick={() => { setSearchValue('2017年第一季度的流动资产合计用柱状图表示') }}>2017年第一季度的流动资产合计用柱状图表示</div>
            <div className="el-upload__tip tipBoxQusetion" onClick={() => { setSearchValue('2018年第二季度的商誉用折线图表示') }}>2018年第二季度的商誉用折线图表示</div>
            <div className="el-upload__tip tipBoxQusetion" onClick={() => { setSearchValue('2018年6月的货币资金是多少') }}>2018年6月的货币资金是多少</div>
          </div>
          {
            list.map((i: any, index: any) => {
              return <div key={index}>

                {i.type == 'left' && <div className='content left'>
                  <div className='chat leftChat'>已经生成图表大屏,<span onClick={() => { toDetails(i.url) }}>请点击查看</span></div>
                </div>}
                {i.type == 'right' && <div className='content right'>
                  <div className='chat rightChat'>{i.value}</div>
                </div>}
                {i.type == 'list' && <div className='checkBox'>
                  <div className='title'>这些指标可能跟您的问题有关，可以作为分析组件组装故事版：</div>
                  <div className='tableBox'>
                    <div className='tableItem line'>
                      {
                        checkAllStatus ? <Icons.CheckboxOn
                          className="checkIcon"
                          iconSize="xl"
                    ...
                    <div className='checkScroll'>
                      {
                        checkList.map((i: any, index: any) => {
                          return <div className='tableItem' key={index}>
                            {
                              i.checkStatus ? <Icons.CheckboxOn
                                className="checkIcon"
                                iconSize="xl"
                                onClick={() => { isCheck(false, index) }}
                              /> : <Icons.CheckboxOff
                                className="checkIcon"
                                iconSize="xl"
                                onClick={() => { isCheck(true, index) }}
                              />
                            }
                            <div>{i.title}</div>
                          </div>
                  ...
                  <div className='footer'>
                    <button type="button" className="btn btn-primary btn-sm" onClick={() => { submitList() }}>确定</button>
                  </div>
      ...
      <InputBox>
        ...
        <button type="button" className="btn btn-primary" onClick={() => { getList() }}>发送</button>
      </InputBox>
      {/* <TemplateContainer>
